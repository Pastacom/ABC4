...
public:
    ...
    // Количество пациентов в больнице.
    // Необходимо для определения конца работы потоков.
    inline static size_t patientsAmount = 0;
    // Метод для добавления пациента в очередь регистрации.
    // Используется при вводе исходных данных.

    // Мьютекс, для корректного работы потоков во время обращения к количеству элементов.
    inline static pthread_mutex_t decrementMutex{};
    ...
    static void *distributePatient(void *) {
        ...
        while (registrationQueue->callPatientChecker()) {
    ...
    static void *callPatient(void *option) {
        while (patientsAmount != 0) {
            ...
            if (queue->callPatientChecker()) {
                queue->callPatient();
                // Критическая секция с декрементом количества пациентов.
                pthread_mutex_lock(&decrementMutex);
                --patientsAmount;
                pthread_mutex_unlock(&decrementMutex);
            }
        }
        return nullptr;
    }
...
int main(int argc, char **argv) {
    ...
    Hospital::patientsAmount = Hospital::registrationQueue->getSize();
    pthread_mutex_init(&Hospital::decrementMutex, nullptr);
    // Массив потоков.
    pthread_t threads[5]{};
